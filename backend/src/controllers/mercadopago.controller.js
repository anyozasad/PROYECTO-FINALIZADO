...
async function crearPreferencia(req, res, next) {
  try {
    ...
    const preferencia = {
      ...
      back_urls: {
        success: `${frontendUrl}/s/checkout?mp=success&venta_id=${venta.venta_id}`,
        failure: `${frontendUrl}/s/checkout?mp=failure&venta_id=${venta.venta_id}`,
        pending: `${frontendUrl}/s/checkout?mp=pending&venta_id=${venta.venta_id}`,
      },
